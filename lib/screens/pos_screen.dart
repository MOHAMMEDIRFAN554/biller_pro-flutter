...
class _PosScreenState extends State<PosScreen> {
  ...
  double get grandTotal => (subTotal - _globalDiscount).roundToDouble();
  double get roundOff => grandTotal - (subTotal - _globalDiscount);
    ...
    showModalBottomSheet(
      ...
      builder: (context) => StatefulBuilder(
        builder: (context, setSheetState) {
          if (_paymentControllers.length != _payments.length) {
            for (var c in _paymentControllers) {
              c.dispose();
            }
            _paymentControllers.clear();
            for (var p in _payments) {
              _paymentControllers.add(TextEditingController(text: p.amount > 0 ? p.amount.toString() : ''));
            }
          }
                            ...
                            children: [
                              ...
                              Expanded(
                                child: TextField(
                                  ...
                                  controller: _paymentControllers[idx],
                                  onChanged: (val) {
                                    double amt = double.tryParse(val) ?? 0;
                                    _payments[idx] = Payment(mode: p.mode, amount: amt, reference: p.reference, showQr: p.showQr);
                                    setState(() {});
                                    setSheetState(() {});
                                  },
                                ),
                ...
                Row(
                  ...
                  children: [
                    TextButton.icon(
                      onPressed: () {
                        setSheetState(() {
                          _payments.add(Payment(mode: 'UPI', amount: balance > 0 ? balance : 0));
                          // Controllers will be sync'd by the builder at the top
                        });
                        setState(() {});
                      },
                  ...
                  child: Column(
                    children: [
                      ...
                      Row(
                        mainAxisAlignment: MainAxisAlignment.spaceBetween,
                        children: [
                          const Text('Extra Discount', style: TextStyle(fontSize: 14, color: Colors.grey)),
                          SizedBox(
                            width: 100,
                            child: TextField(
                              decoration: const InputDecoration(isDense: true, prefixText: '₹'),
                              ...
                              keyboardType: TextInputType.number,
                              onChanged: (val) => setState(() => _globalDiscount = double.tryParse(val) ?? 0),
                            ),
                          ),
                        ],
                      ),
